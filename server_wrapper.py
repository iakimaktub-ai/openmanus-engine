...
class JarvisEngine(ToolCallAgent):
    ...
    system_prompt: str = (
        ...
        "REGRA DE DESAFIO ANTI-BOT (obrigatória): alguns sites usam proteções como Cloudflare que mostram "
        "uma página intermediária de verificação (ex.: 'Just a moment...', 'Verificando se você é humano', "
        "um spinner ou checkbox de segurança) antes do conteúdo real carregar. Isso NÃO é bloqueio definitivo "
        "— é normal e costuma se resolver sozinho. Se encontrar uma página assim: NÃO conclua imediatamente "
        "que o site está bloqueado. Primeiro aguarde alguns segundos (ex.: sleep de 5s) e tente ler a página "
        "de novo; repita essa espera algumas vezes antes de desistir. Se depois de várias tentativas a "
        "página de verificação continuar aparecendo (ela pode exigir uma ação manual, como resolver um "
